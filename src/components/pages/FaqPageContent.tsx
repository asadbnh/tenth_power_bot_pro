    ...
    <div className="pt-[var(--header-height)] min-h-dvh bg-gradient-to-b from-background to-surface">
      ...
      <section className="relative pt-8 pb-12 sm:pt-12 sm:pb-16 bg-slate-50 dark:bg-[#070d1e] overflow-hidden border-b border-slate-200/80 dark:border-amber-500/10 transition-colors duration-300">
        ...
        <div className="absolute inset-0 z-0 pointer-events-none">
          <PageHeroBackground pageKey="faq" overlayOpacity={0.4} />
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-4 sm:space-y-5">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            ...
            <Search className={cn("absolute top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-white/50", isRtl ? "right-4" : "left-4")} />
            <input type="text" value={query} onChange={(e) => setQuery(e.target.value)}
              placeholder={isRtl ? "ابحث في الأسئلة الشائعة والمواصفات..." : "Search in FAQs and specifications..."}
              className={cn(
                "w-full py-3.5 rounded-xl bg-white dark:bg-white/10 border border-slate-300 dark:border-white/20 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-white/50 text-sm shadow-sm dark:shadow-none focus:outline-none focus:ring-2 focus:ring-amber-500 backdrop-blur-md transition-all",
