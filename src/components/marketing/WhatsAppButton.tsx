"use client";

import { useState, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { X } from "lucide-react";
import { cn } from "@/lib/utils";

interface WhatsAppButtonProps {
  phoneNumber?: string;
  message?: string;
  locale?: string;
}

/**
 * Floating WhatsApp button with pulse animation and tooltip.
 * Positioned bottom-end (bottom-right LTR, bottom-left RTL).
 */
export function WhatsAppButton({
  phoneNumber = "+966500000000",
  message,
  locale = "ar",
}: WhatsAppButtonProps) {
  const [isTooltipVisible, setIsTooltipVisible] = useState(false);
  const [hasDismissed, setHasDismissed] = useState(false);
  const isRtl = locale === "ar";

  // Show tooltip after a delay on first visit
  useEffect(() => {
    const dismissed = sessionStorage.getItem("wa_tooltip_dismissed");
    if (dismissed) {
      setHasDismissed(true);
      return;
    }

    const timer = setTimeout(() => {
      setIsTooltipVisible(true);
    }, 5000);

    return () => clearTimeout(timer);
  }, []);

  const dismissTooltip = () => {
    setIsTooltipVisible(false);
    setHasDismissed(true);
    sessionStorage.setItem("wa_tooltip_dismissed", "true");
  };

  const defaultMessage =
    locale === "ar"
      ? "مرحبًا، أود الاستفسار عن خدماتكم"
      : "Hello, I would like to inquire about your services";

  const whatsappUrl = `https://wa.me/${phoneNumber.replace(/[^\d+]/g, "")}?text=${encodeURIComponent(message || defaultMessage)}`;

  return (
    <div
      className={cn(
        "fixed bottom-6 z-40 no-print",
        isRtl ? "start-6" : "end-6"
      )}
      style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)" }}
    >
      {/* Tooltip */}
      <AnimatePresence>
        {isTooltipVisible && !hasDismissed && (
          <motion.div
            initial={{ opacity: 0, y: 10, scale: 0.9 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 10, scale: 0.9 }}
            className={cn(
              "absolute bottom-full mb-3 bg-white dark:bg-surface-elevated",
              "rounded-xl shadow-xl border border-border p-3",
              "min-w-[200px]",
              isRtl ? "start-0" : "end-0"
            )}
          >
            <button
              onClick={dismissTooltip}
              className="absolute top-2 end-2 text-text-tertiary hover:text-text-primary"
              aria-label="Close"
            >
              <X className="w-3.5 h-3.5" />
            </button>
            <p className="text-sm font-medium text-text-primary pe-4">
              {isRtl ? "تحتاج مساعدة؟" : "Need help?"}
            </p>
            <p className="text-xs text-text-secondary mt-1">
              {isRtl
                ? "تواصل معنا عبر واتساب"
                : "Chat with us on WhatsApp"}
            </p>
            {/* Tooltip arrow */}
            <div
              className={cn(
                "absolute -bottom-1.5 w-3 h-3 rotate-45",
                "bg-white dark:bg-surface-elevated border-b border-e border-border",
                isRtl ? "start-5" : "end-5"
              )}
            />
          </motion.div>
        )}
      </AnimatePresence>

      {/* WhatsApp Button */}
      <motion.a
        href={whatsappUrl}
        target="_blank"
        rel="noopener noreferrer"
        className={cn(
          "flex items-center justify-center w-14 h-14 rounded-full",
          "bg-[#25D366] text-white shadow-lg",
          "hover:bg-[#20BD5A] hover:shadow-xl hover:scale-110",
          "active:scale-95 transition-all duration-200"
        )}
        aria-label={isRtl ? "تواصل عبر واتساب" : "Chat on WhatsApp"}
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 0.95 }}
        onClick={dismissTooltip}
      >
        {/* WhatsApp SVG Icon */}
        <svg
          viewBox="0 0 24 24"
          width="28"
          height="28"
          fill="currentColor"
        >
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
        </svg>

        {/* Pulse ring */}
        <span className="absolute inset-0 rounded-full animate-ping bg-[#25D366]/40 pointer-events-none" />
      </motion.a>
    </div>
  );
}
